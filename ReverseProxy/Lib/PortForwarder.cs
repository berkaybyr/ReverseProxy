...
namespace ReverseProxy.Lib
{
    public class PortForwarder
    {
        ...
        public PortForwarder(ReverseProxyConfig config)
        {
            ...
            Server = new TcpListener(IPAddress.Parse(config.Host.Ip), config.Host.Port);
        }
        ...
        private void RunServer()
        {
            ...
            Console.WriteLine("Listening from {0}:{1} forwarded to {2}:{3}",Config.Host.Ip,Config.Host.Port,Config.Forward.Ip,Config.Forward.Port);
        }
        ...
        private void HandleClient(ClientInfo info)
        {
            ...
            destClient.BeginConnect(Config.Forward.Ip, Config.Forward.Port, EndConnectWriter, info);
        }
